...
/**
 * MenuXChartPanel is an extension of the XChartPanel from org.knowm.xchart where the pop-up menu has been made more accessible,
 * amongst others to maximize the view of a chart.
 * <p>
...
public class MenuXChartPanel<T extends Chart<?, ?>> extends XChartPanel<T> implements EventListener
{
                ...
                try
                {
                    ...
                    else if (fileChooser.getFileFilter().getDescription().equals("*.jpg,*.JPG"))
                    {
                        BitmapEncoder.saveJPGWithQuality(getChart(),
                                BitmapEncoder.addFileExtension(theFileToSave.getCanonicalPath(), BitmapFormat.JPG), 1.0f);
                    }
    ...
    private void showExportAsDialog()
    {
        ...
        if (fileChooser.showSaveDialog(null) == JFileChooser.APPROVE_OPTION)
        {
            ...
            try
            {
                CSVExporter.writeCSVColumns((XYChart) getChart(), theFileToSave.getCanonicalPath() + File.separatorChar);
            }
            catch (IOException e)
            {
                e.printStackTrace();
    ...
    /**
     * File filter based on the suffix of a file. This file filter accepts all files that end with .suffix or the capitalized
     * suffix.
    ...
    private static class SuffixSaveFilter extends FileFilter
    {
        ...
        /**
         * This file filter accepts all files that end with .suffix or the capitalized suffix.
