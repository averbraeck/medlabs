...
/**
 * LocationAnimationRect2x1 implements a rectangle location animation with
 * x-size 4 and y-size 2, with respect to the provided half-width.
 * <p>
 ...
 */
public class LocationAnimationRect1x2 extends LocationAnimation {
	/** */
	private static final long serialVersionUID = 20210102L;

	/**
	 * Rectangle animation for a location. The x-size is 4 and y-size is 2, with
	 * respect to the provided half-width.
	 * 
	 * @param location Location; the location to be animated
	 * @param template LocationAnimationTemplate; the template to use
	 */
	public LocationAnimationRect1x2(final Location location, final LocationAnimationTemplate template) {
	...
	public void paint(final Graphics2D graphics, final ImageObserver observer) {
		...
		if (this.template.getFillColor() != null) {
			...
			graphics.fillRect(-half, -x2, x2, x4);
		}
		graphics.setColor(this.template.getLineColor());
		graphics.drawRect(-half, -x2, x2, x4);
		...
		graphics.drawString(number, half + 2, x2);
	}
