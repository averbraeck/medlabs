...
/**
 * Abstract class from which a disease model for a certain city or region can be
 * extended.
 * <p>
...
public abstract class AbstractMedlabsModel extends AbstractDsolModel<Double, SimpleDEVSSimulatorInterface>
	...
	/**
	 * the standard uniform distribution based on the standard random stream of the
	 * model.
	 */
	...
	/**
	 * the family compositions in the model (array of person ids), indexed by home
	 * location.
	 */
	 ...
	 * 
	 * @param simulator        SimpleDEVSSimulatorInterface; the simulator for this
	 *                         model
	...
	/**
	 * Make the initial input parameter ma with a few parameters that are used by
	 * the generic MedlabsModel such as the name for the experiment, the output path
	 * for the results (or write no results at all), and the need for animation. The
	 * method extendInputParameterMap() is called at the end, enabling extensions of
	 * this abstract model to easily add parameters.
	 */
	public void makeInputParameterMap() {
		try {
			...
			genericMap.add(
					new InputParameterBoolean("WriteOutput", "Write output?", "Output writing on or off", true, 2.0));
			...
			genericMap.add(new InputParameterInteger("RunLength", "Run length in days", "Run length in days", 60, 1,
					1000, "%d", 4.0));
			genericMap.add(new InputParameterLong("Seed", "Seed for the RNG", "Seed for the Random Number Generator",
					111L, 1, Long.MAX_VALUE, "%d", 5.0));
	...
	/**
	 * Extend the input parameters with extra tabs and parameters. The base
	 * parameters are already there.
	 * 
	...
	/**
	 * This method should be implemented in the final model to construct the actual
	 * model.
	 */
	protected abstract void constructModelFromSource();

	/**
	 * Check for all persons whether they need to change their day pattern or not.
	 * This method can be overridden to specify a custom method. In that case, don't
	 * forget to schedule the method again after 24 hours!
	 */
