...
/**
 * The AgeProbabilityParser utility class can parse a string with probabilities
 * for age groups. The format is, e.g.,
 * 
 * <pre>
...
public final class AgeProbabilityParser {
	...
	/**
	 * Parse a string with an age-dependent probability. Intervals that have not
	 * been specified will have a probability of 0.0. The string is parsed
	 * left-to-right. Overlaps do not matter -- so you can specify: age{0-100: 0.5,
	 * 20-39: 0.75} to indicate a 0.5 probability for all ages except 20-39
	 * (inclusive) that have a 0.75 probability.
	 * 
	 * @param probStr String; the textual representation of the probabilities
	 * @return Map&lt;Integer, Double&gt;; a complete map from age to probability
	 *         for 0-100 years.
	...
	public static Map<Integer, Double> parseAgeProbabilities(final String probStr) throws MedlabsException {
		if (!probStr.startsWith("age{"))
			throw new MedlabsException(
					"parsing age-probabilities '" + probStr + "': string does not start with 'age{'");
		...
		for (String arg : argArray) {
			if (arg.indexOf(":") == -1)
				throw new MedlabsException(
						"parsing age-probabilities '" + probStr + "': has entry " + arg + " without ':'");
	...
	private static double argDouble(final String probStr, final String arg) throws MedlabsException {
		try {
			...
			throw new MedlabsException(
					"parsing age-probabilities '" + probStr + "': entry " + arg + " is not a double");
		}
	}
