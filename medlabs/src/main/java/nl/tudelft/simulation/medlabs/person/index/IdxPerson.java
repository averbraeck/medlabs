 ...
 * a disease status, and an activity pattern. The IdxPerson is implemented using
 * int indexes as much as possible, saving 4 bytes of memory for each attribute
 * as compared to a pointer to an object.
 * <p>
...
public class IdxPerson extends AbstractPerson {
	...
	/**
	 * index of current week pattern, can be standard or adapted pattern due to
	 * lockdown or illness.
	 */
	...
	/**
	 * standard day pattern, to be able to return to 'normal' after lockdown or
	 * illness.
	 */
	@SuppressWarnings("checkstyle:visibilitymodifier")
	...
	/**
	 * index of the current activity in the week pattern. -1 means dummy activity
	...
	/**
	 * sublocation index of the home where the person lives (to get families living
	 * together).
	 */
	@SuppressWarnings("checkstyle:visibilitymodifier")
	 ...
	 * machine is started for the person if needed. The week pattern starts at day 0
	 * and activity index 0. In the latest version, persons are not initialized with
	 * a Disease anymore -- the disease is set to certain persons AFTER the
	...
	public void init() {
		...
		try {
			this.model.getSimulator().scheduleEvent(new TinySimEvent(this.model.getSimulator().getSimulatorTime(), this,
					this, "executeStartOfActivity", null));
	...
	public void endActivity() {
		...
		// NOTE: the startActivity() method SHOULD be the last statement of
		// endActivity()
	...
	public void setCurrentWeekPattern(final WeekPattern currentWeekPattern) {
		// TODO: set the activity to -1 to avoid problems with retrieving the previous
		// activity
		// This does not work...
		// getCurrentLocation().removePerson(this);
