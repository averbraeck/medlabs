 ...
 * for the naming of a 'register'). Several types of Registers are available in
 * the software (and these can be extended further):
 * <ul>
 * <li>Register interface, containing the 'register(person)' method and the
 * isRegistered(person) method.</li>
 * <li>PersonRegister class, persistently containing that a person has been
 * registered. Registry is once. An example is a model with a single illness and
 * lifelong immunity, where registration of having had the disease is
 * sufficient.</li>
 * <li>LastDateRegister class, persistently containing the date that a person
 * has been registered. The last date is kept. An example is an official test
 * where the last day of the test is kept.</li>
 * <li>MultipleDateRegister class, persistently containing the dates that a
 * person has been registered. Multiple dates can be registered for the same
 * person. An example is the registration of multiple vaccinations and booster
 * shots.</li>
 * <li>TimeoutDateRegister class, containing the last date that a person has
 * been registered, with a timeout on how long the record is kept. An example is
 * a self test that is valid for 2 days. After 2 days, the record is erased
 * automatically.</li>
