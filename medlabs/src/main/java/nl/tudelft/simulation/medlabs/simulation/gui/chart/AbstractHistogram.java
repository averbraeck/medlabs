...
/**
 * AbstractHistogram is the parent class for a histogram that can display
 * counted values per bin. It embeds a chart that can display the histogram on
 * the screen. Currently we use XCharts for the display, but it could easily be
 * changed for another charting package.
 * <p>
 ...
 * @param <T> the type of variable that defines the bins; typically String,
 *            Integer or Double
 */
public abstract class AbstractHistogram<T> extends DynamicChart<CategoryChart> {
	...
	public AbstractHistogram(final String title, final String xLabel, final String yLabel) {
		this.chart = new CategoryChartBuilder().title(title).xAxisTitle(xLabel).yAxisTitle(yLabel).width(1920)
				.height(1280).build();
	...
	/**
	 * Resolve the bin number for value x. Use the bin with index nrBins is used for
	 * 'wrong' x-values.
	 * 
	 * @param x T; the value to resolve the bin for
	 * @return the bin number, or nrBins in case the bin for value x could not be
	 *         resolved
	 */
	 ...
	 * @return int; the value of the count of the bin, or -1 if the value does not
	 *         translate to a bin
	 */
