...
/**
 * Default implementation of a number of Properties arrays, to which the model
 * can delegate the implementation for instance for Persons or Locations. The
 * Properties classs is therefore a delegated class from the Model. You can then
 * ask, e.g.,
 * <code>model.getPersonProperties().getIntValue(personId, "age");</code><br>
 * Storage for the types of property arrays is optimized for speed and not for
 * memory-- one map exists for each type to avoid casting for every addition and
 * every retrieval. Since an empty map is not very big, this is not considered
 * to be a problem.
 * <p>
...
public class Properties {
	...
	/**
	 * Create a map of properties for a certain object, where all properties have
	 * the same size.
	 * 
	 * @param objectClass Class&lt;?&gt;; the object class for which this is the
	 *                    property
	 ...
	 * 
	 * @return String; the name of the object class for which the arrays are
	 *         maintained
	 */
