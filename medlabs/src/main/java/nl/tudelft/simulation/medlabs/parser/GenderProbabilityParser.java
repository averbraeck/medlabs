...
/**
 * The genderProbabilityParser utility class can parse a string with probabilities that differ per gender. The format is, e.g.,
 * 
 * <pre>
...
public final class GenderProbabilityParser
{
    ...
    /**
     * Parse a string with a gender-dependent probability. It always has to have the format gender{M: 0.021, F: 0.016}. The
     * result can be used based on person.genderFemale as follows:
     * 
     * <pre>
     ...
     * @return double[2]; where [0] indicates the probability for female, and [1] the probability for male
    ...
    public static double[] parseGenderProbabilities(final String probStr) throws MedlabsException
    {
        if (!probStr.startsWith("gender{"))
            throw new MedlabsException("parsing gender-probabilities '" + probStr + "': string does not start with 'gender{'");
        ...
        {
            if (arg.indexOf(":") == -1)
                throw new MedlabsException("parsing gender-probabilities '" + probStr + "': has entry " + arg + " without ':'");
            ...
            else if (arg2[0].toUpperCase().trim().equals("M"))
            {
                ...
                throw new MedlabsException("parsing gender-probabilities '" + probStr + "': has entry " + arg + ", not M or F");
        }
    ...
    private static double argDouble(final String probStr, final String arg) throws MedlabsException
    {
        ...
        {
            throw new MedlabsException("parsing gender-probabilities '" + probStr + "': entry " + arg + " is not a double");
        }
    }
}
