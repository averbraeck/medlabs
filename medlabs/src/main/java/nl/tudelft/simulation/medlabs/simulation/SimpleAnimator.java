...
public class SimpleAnimator extends DevsRealTimeAnimator<Double>
	...
	/**
	 * the translation from a millisecond on the wall clock to '1.0' in the
	 * simulation time. This means that if the wall clock runs in seconds, the
	 * factor should be 0.001 if we want a real-time model, and if the simulation
	 * time is in hours, the factor should be 1.0 / (1000.0 * 3600.0).
	 */
	private final double msecWallClockToSimTimeUnit;

	/**
	 * The time unit of the simulator is in hours. The default setting for animation
	 * speed "1" is now set to 0.1 hour/sec, which is 0.1 / 1000.0 hours / msec.
	 * 
	...
	public void initialize(final double startTime, final double warmupPeriod, final double runLength,
		...
		SimpleReplication newReplication = new SimpleReplication("rep" + replicationNr, startTime, warmupPeriod,
				runLength);
