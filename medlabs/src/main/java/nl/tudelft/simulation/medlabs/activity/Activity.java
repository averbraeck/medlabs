...
/**
 * The abstract Activity is the root class to indicate activities of persons in
 * a day pattern.
 * <p>
...
public abstract class Activity extends AbstractModelNamed {
	...
	/**
	 * indicate that this is an activity that should be started even when it is
	 * after midnight (default false).
	 */
	 ...
	 * @param activityLocator LocatorInterface; the object that can determine the
	 *                        location where the activity takes place (e.g., to
	 *                        calculate statistics), usually this is the
	 *                        CurrentLocator
	 */
	...
	/**
	 * Create an Activity that has a start locator (almost always the
	 * CurrentLocator), a locator to determine the location where movement takes
	 * place (a car, bike, public space for on foot, or public transport), and a
	 * locator to calculate the arrival location.
	 * 
	 * @param model           MedlabsModelInterface; pointer to the model for
	 *                        retrieving simulator and other relevant information
	 * @param name            String; the name of the activity
	 * @param activityLocator LocatorInterface; the locator to determine the
	 *                        activity location
	 * @param startLocator    LocatorInterface; the locator to determine the start
	 *                        location (usually the CurrentLocator)
	 * @param endLocator      LocatorInterface; the locator to determine the arrival
	 *                        location
	 */
	public Activity(final MedlabsModelInterface model, final String name, final LocatorInterface activityLocator,
	...
	/**
	 * Finish the activity for a given person. Gather statistics on the location
	 * where the person has been. Start the next activity of the person.
	 * 
	...
	/**
	 * Return the activity duration in hours. When the duration is not fixed, and
	 * e.g. dependent on transport activities or social activities, this method
	 * should return Double.NaN.
	 * 
	 * @param person Person; the person for whom the activity duration has to be
	 *               calculated
	 ...
	 * 
	 * @param startAfterMidnight boolean; whether the activity should start after
	 *                           midnight
	 */
	 ...
	 * 
	 * @param person Person; the person for whom the extended information has to be
	 *               returned
