...
/**
 * The WeekPattern contains seven DayPatterns for a type of Person. This implementation uses a maximum of 4096 slots per day, so
 * the <code>weekIndex = day * 4096 + dayIndex</code>, and <code>dayIndex = weekIndex & 0xfff</code>.
 * <p>
...
public class WeekDayPattern extends AbstractModelNamed implements WeekPattern
{
    ...
    /**
     * Create a WeekPatttern where all days are the same. The WeekPattern will be registered in the Model.
     * @param model MedlabsModelInterface; the model
    ...
    /**
     * Create a WeekPatttern with weekdays and weekend days. The WeekPattern will be registered in the Model.
     * @param model MedlabsModelInterface; the model
     * @param name String; the week pattern name
    ...
    /**
     * Create a WeekPatttern where all days are different. The WeekPattern will be registered in the Model.
     * @param model MedlabsModelInterface; the model
     * @param name String; the week pattern name
     * @param dayPatterns DayPattern[7]; different patterns for 7 days, starting with Monday
     */
    ...
    public int getNextActivityIndex(final Person person, final int prevActivityIndex)
    {
        ...
        {
            // there are no more activities for the current day -- fill with dummy activity till the end of the day
            return -1;
        }
        if (prevWeekDay != weekDay)
        {
            ...
            // otherwise: if the next activity is a 'rollover' one, just start it, although it is out of sync
