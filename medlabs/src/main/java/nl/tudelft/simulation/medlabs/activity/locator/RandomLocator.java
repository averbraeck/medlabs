...
/**
 * The RandomLocator returns a random location of a certain type within a
 * certain distanc, e.g., a restaurant within a 2 kilometer radius.
 * <p>
...
public class RandomLocator implements LocatorInterface {
	...
	/**
	 * Construct a locator that draws a random Location of a certain type within a
	 * search radius.
	 * 
	 * @param startLocator         LocationInterface&lt;T&gt; the starting position
	 *                             to which the other location needs to be found
	 * @param activityLocationType the cumulative probabilities and corresponding
	 *                             location types to return
	 * @param maxDistanceM         double; the search radius in meters
	 * @param reproducible         boolean; whether the draw should be reproducible
	...
	public RandomLocator(final LocatorInterface startLocator, final LocationType activityLocationType,
		...
		this.seed = this.activityLocationType.getModel().getDefaultStream().getOriginalSeed()
	...
	public Location getLocation(final Person person) {
		...
		if (locations.length == 0) {
			...
			int index = this.reproducible
					? model.getReproducibleJava2Random().nextInt(0, locations.length, (person.hashCode()))
		...
		if (this.activityLocationType.getFractionActivities() < 1.0
				|| this.activityLocationType.getFractionOpen() < 1.0) {
