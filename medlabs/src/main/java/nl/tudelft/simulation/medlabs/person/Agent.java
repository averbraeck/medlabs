...
/**
 * The Agent is the base entity in the simulation model. To avoid static
 * variables, the Agent does have a reference to the model, so it can look up
 * activities, week patterns, diseases, and locations. An agent is identified by
 * a unique id (int). Ids are used in arrays, so sequential numbering of the id
 * from 0 onward is preferred.
 * <p>
 * Copyright (c) 2020-2024 Delft University of Technology, Jaffalaan 5, 2628 BX
 * Delft, the Netherlands. All rights reserved. The EPILABS project (Epidemic
 * disease modeling with Large-scale Agent-Based Simulation) is aimed at
