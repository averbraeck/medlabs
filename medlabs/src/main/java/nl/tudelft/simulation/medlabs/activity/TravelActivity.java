...
/**
 * The TravelActivity is an activity that has a location through a Locator
 * (walk, bike, car, bus, metro, etc.) where the move takes place, and a
 * startLocator and endLocator. E.g., get from the house to the closest
 * supermarket by bike.
 * <p>
...
public abstract class TravelActivity extends Activity {
	...
	/**
	 * Create a travel activity that has a start locator (almost always the
	 * CurrentLocator), a locator to determine the location where movement takes
	 * place (a car, bike, public space for on foot, or a public transport means),
	 * and a locator to calculate the arrival location.
	 * 
	 ...
	 * @param travelLocator LocatorInterface; the locator to determine the location
	 *                      in which travel takes place
	...
	public void startActivity(final Person person) {
		...
		// we have to report the time here since we don't know anymore when the activity
		...
		this.model.getSimulator()
				.scheduleEvent(new TinySimEvent(this.model.getSimulator().getSimulatorTime() + activityDuration, this,
						this, "finishTravelActivity", new Object[] { person, travelLocation, toLocation }));
	}
	 ...
	 * @param travelLocation Location; the current location of the person (bus,
	 *                       metro, walk, bike, car, etc.)
	 * @param toLocation     Location; the location at the end of the travel
	 *                       activity
	 */
