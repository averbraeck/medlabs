...
/**
 * The NearestLocatorChoise locator draws a type of location to return with a
 * probability, and then returns the closest location of that type relative to
...
public class NearestLocatorChoice implements LocatorInterface {
	 ...
	 * the nearest location of that type.
	 * 
	...
	public NearestLocatorChoice(final LocatorInterface startLocator,
		...
		this.seed = this.activityLocationTypeMap.values().iterator().next().getModel().getDefaultStream()
				.getOriginalSeed() + "NearestLocatorChoice".hashCode();
	...
	public Location getLocation(final Person person) {
		...
		for (Map.Entry<Double, LocationType> entry : this.activityLocationTypeMap.entrySet()) {
			if (prob < entry.getKey()) {
				if (entry.getValue().getLocationTypeId() == person.getModel().getLocationTypeHouse()
						.getLocationTypeId()) {
