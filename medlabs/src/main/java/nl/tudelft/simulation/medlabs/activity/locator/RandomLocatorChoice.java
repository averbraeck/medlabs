...
/**
 * The RandomLocatorChoise locator draws a type of location to return with a
 * probability, and then returns a random location of that type within a certain
 * distance, e.g., a restaurant within a 2 kilometer radius.
 * <p>
...
public class RandomLocatorChoice implements LocatorInterface {
	...
	/**
	 * Construct a locator that draws a LocationType with a probability and returns
	 * a random location of that type within a certain radius.
	 * 
	 * @param startLocator            LocationInterface&lt;T&gt; the starting
	 *                                position to which the other location needs to
	 *                                be found
	 * @param activityLocationTypeMap the cumulative probabilities and corresponding
	 *                                location types to return
	 * @param maxDistanceM            double the maximum distance in meters
	 * @param reproducible            boolean; whether the draw should be
	 *                                reproducible for the person or not
	 */
	public RandomLocatorChoice(final LocatorInterface startLocator,
		...
		this.seed = this.activityLocationTypeMap.values().iterator().next().getModel().getDefaultStream()
				.getOriginalSeed() + "RandomLocator".hashCode();
	...
	public Location getLocation(final Person person) {
		...
		double prob = this.reproducible
				? person.getModel().getReproducibleJava2Random().nextDouble(person.hashCode() + 1)
