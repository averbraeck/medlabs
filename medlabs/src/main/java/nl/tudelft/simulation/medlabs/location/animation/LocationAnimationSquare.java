...
/**
 * LocationAnimationSquare implements a square location animation.
 * <p>
 ...
 */
public class LocationAnimationSquare extends LocationAnimation {
	/** */
	private static final long serialVersionUID = 20210102L;

	/**
	 * Square animation for a location.
	 * 
	 * @param location Location; the location to be animated
	 * @param template LocationAnimationTemplate; the template to use
	 */
	public LocationAnimationSquare(final Location location, final LocationAnimationTemplate template) {
	...
	public void paint(final Graphics2D graphics, final ImageObserver observer) {
		...
		if (this.template.getFillColor() != null) {
			...
			graphics.fillRect(-half, -half, full, full);
		}
		graphics.setColor(this.template.getLineColor());
		graphics.drawRect(-half, -half, full, full);
