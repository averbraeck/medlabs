...
public class MedlabsPanel extends DsolPanel {
				...
				for (LocationType locationType : getModel().getLocationTypeIndexMap().values()) {
					if (locationType.getAnimationClass() != null) {
						...
						for (Location location : locationType.getLocationMap().values(new Location[0])) {
							animationClass.getDeclaredConstructor(new Class<?>[] { Location.class })
	...
	/**
	 * Make two tabs with location statistics: one for the number of people per
	 * location, the other for the duration per location.
	 */
	protected void addLocationStatistics() {
		...
		for (LocationType locationType : getModel().getLocationTypeIndexMap().values()) {
			try {
				...
				chart.getChart().getStyler().setPlotMargin(0).setXAxisDecimalPattern("###0")
						.setYAxisDecimalPattern("#####0").setYAxisLabelAlignment(TextAlignment.Right)
	...
	private void addPersonStatistics() {
		try {
			...
			HistogramInt familySizeHistogram = new HistogramInt("Family size distribution", "family size", "#", 0, 9, 1,
					false);
