...
/**
 * Calculation that each location carries out when people leave or enter the
 * location. When there are infectious persons in the location, when he/she has
 * a chance of getting infected, to determine whether infection actually takes
 * place.<br>
 * <br>
 * The DiseaseTransmission model used two caches to speed up the infection
 * calculations. The first cache contains people present in sublocations for the
 * sublocations with one or more infectious persons. The key of the internal Map
 * is a Long where the first 32 bits are used for the location id, and the last
 * 32 bits for the sublocation number. The value is an ArrayList of Person ids.
 * The Map takes a considerable amount of memory but is can sped up calculations
 * with a factor 500 or more... <br>
 * <br>
 * The second cache contains the last calculation time for a location where one
 * or more infectious persons are present. Note that locations where no
 * infectious persons are located do not need to store such a time.
 * <p>
...
public abstract class DiseaseTransmission extends AbstractModelNamed implements Serializable {
	...
	/**
	 * Cache for the sublocations where infectious persons are present to speed up
	...
	/**
	 * Cache with the last time when disease transmission has been calculated for a
	 * sublocation. Stored here instead of an array at each location, which is not
	 * needed for those sublocations where no infected people are present.
	 */
	...
	/**
	 * This method is called when a person enters a location. When there are no
	 * infectious persons, the method returns fast. The method can also return
	 * quickly when the delta-time is very short (e.g, less than a minute but be
	 * aware that spread in public transport and shops might suffer since many
	 * people arrive and depart with short intervals). Note that the method can
	 * maintain the attribute infectiousPersonPresent in Location to speed up the
	 * calculations.
	 * 
	 * @param location         Location; the location for which to calculate
	 *                         infectious spread
	...
	/**
	 * This method is called when a person leaves a location. When there are no
	 * infectious persons, the method returns fast. The method can also return
	 * quickly when the delta-time is very short (e.g, less than a minute but be
	 * aware that spread in public transport and shops might suffer since many
	 * people arrive and depart with short intervals). Note that the method can
	...
	public void calculateTransmissionLeave(final Location location, final short subLocationIndex, final Person person) {
		...
		if (infectPeople(location, persons,
				this.simulator.getSimulatorTime().doubleValue() - getLastCalculationTime(key))) {
			updateLastCalculationTime(key);
		}
		persons.remove(person.getId());

		// Check whether there are still infectious persons in the location after this
		// person left
		// Note that we cannot just base this on the person who leaves because a) more
		// than one infectious person may be
		// present, and b) the status of a person's disease may have changed in the
		// meantime.
		// But when the person who leaves is still susceptible, no recalculation needs
		// to take place.
	...
	/**
	 * Calculate the disease spread for all persons present in this location during
	 * the 'duration' in hours. The method could return quickly when the delta-time
	 * is very short (e.g, less than a minute but be aware that spread in public
	 * transport and shops might suffer since many people arrive and depart with
	 * short intervals).
	 * 
	 * @param location             Location; the location for which to calculate
	 *                             infectious spread
	 * @param personsInSublocation TIntSet; the persons present in the sublocation
	 *                             (one or more are potentially contagious)
	 * @param duration             double; the time for which the calculation needs
	 *                             to take place, in hours
	 * @return boolean; whether the calculation took place and the last calculation
	 *         time can be updated
	 */
	...
	/**
	 * Return whether the sublocation of this location contains one or more infected
	 * persons.
	 * 
	 * @param location         Location; the location to check
	 * @param subLocationIndex int; the sublocation number to check
	 * @return boolean; whether the sublocation of this location contains one or
	 *         more infected persons
	 */
	...
	/**
	 * Return whether the key coding a sublocation and a location contains one or
	 * more infected persons.
	 * 
