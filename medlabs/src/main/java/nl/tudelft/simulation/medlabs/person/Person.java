...
/**
 * This interface describes the contract of a person. Implementations of the
 * behavior of a person can differ with respect to speed and memory usage. A
 * person should be able to:
 * <ul>
 * <li>execute activities based on a day/week pattern</li>
 * <li>start using a different activity pattern based on disease or policy
 * measures</li>
 * <li>get infected with a disease and go through several stages of
 * infection</li>
...
public interface Person extends Locatable, Serializable, PropertiesInterface, ModelIdentifiable {
	/**
	 * Initialize the behavior of the person. This has been separated from the
	 * constructor, since often other classes such as locations and activity
	 * patterns have to be generated as well. The init() method is therefore NOT
	 * called from the constructor and has to be called explicitly. Init() takes
	 * care of: starting the first activity; setting the initial location for that
	 * activity; setting the initial disease state if present; starting the activity
	 * scheduling.
	 */
	void init();

	/**
	 * Execute the current activity, and asynchronously wait for the a call by the
	 * activity to the endActivity method.
	 */
	 ...
	 * 
	 * @param exposureTime float; the exposure time of the person in hours
	 *                     (simulation clock)
	 */
