...
/**
 * DiseasePhase is a class in which a phase of the disease can be registered.
 * Different diseases have different stages or phases we might want to consider.
 * The SEIR model is an often used model (Symptomatic, Exposed, Infected,
 * Recovered) to indicate a number of phases of a transmissable disease. This
 * model can be extended with many other phases, e.g., a symptomatic or
...
public class DiseasePhase {
	 ...
	 */
	public DiseasePhase(final DiseaseProgression disease, final String name, final byte index,
			final DiseaseState diseaseState) {
		this.disease = disease;
		this.name = name;
		this.index = index;
		this.diseaseState = diseaseState;
		this.DISEASE_STATISTICS_EVENT = new EventType("DISEASE_STATISTICS_EVENT_" + disease.getName() + "_" + name,
				new MetaData(name, disease.getName() + "_" + name,
						new ObjectDescriptor("number", "number in phase", Integer.class)));
	}
	...
	/**
	 * Return whether the phase is an 'ill' phase, e.g., exposed, infected,
	 * (a)symptomatic infectious in any stage.
	 * 
	...
	/**
	 * Return whether the phase is a 'dead' phase, so the person can, e.g., be
	...
	public String toString() {
		return "DiseasePhase [disease=" + this.disease + ", name=" + this.name + ", index=" + this.index
