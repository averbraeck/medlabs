 ...
 * Delft, the Netherlands. All rights reserved. The code is a demo for the
 * MEDLABS project. The simulation tools are aimed at providing policy analysis
 * tools to predict and help contain the spread of epidemics. They make use of
 * the DSOL simulation engine and the agent-based modeling formalism. This
 * software is licensed under the BSD license. See license.txt in the main
 * project.
 * </p>
 * 
...
public class ConstructSEIRModel {
	...
	private void makeLocations() throws Exception {
		...
		// make m workplaces numbered 100_000 to 100_000 + m - 1, x m2 with y
		// sublocations.
	...
	private void makeWeekpatternData() throws Exception {
		...
		Activity travel1 = new TravelActivityFly(this.model, "travel1", new CarLocator(), new CurrentLocator(),
				new WorkLocator());
		Activity work = new FixedDurationActivity(this.model, "work", new WorkLocator(), 10.0);
		Activity travel2 = new TravelActivityFly(this.model, "travel2", new CarLocator(), new CurrentLocator(),
				new HomeLocator());
	...
	static class TravelActivityFly extends TravelActivity {
		...
		public TravelActivityFly(final MedlabsModelInterface model, final String name,
