 ...
 * Delft, the Netherlands. All rights reserved. The code is part of the SEIR
...
public class SEIRModel extends AbstractMedlabsModel {
	...
	public SEIRModel(final SimpleDEVSSimulatorInterface simulator, final String propertyFilename) {
		...
		this.locationTypeHouse = new LocationType(this, (byte) 0, "house", Location.class, HouseAnimation.class, true,
				true, 1.0);
	}
	...
	protected void extendInputParameterMap() throws InputParameterException {
		...
		settingsMap.add(new InputParameterInteger("NumberPersons", "number of persons in the model",
				"(between 1 and 50000)", 1000, 1, 50000, "%d", 1.0));
		...
		settingsMap.add(new InputParameterDouble("WorkplaceSize", "total workplace size in m2",
				"(m2, between 1 and 1,000,000)", 100.0, 1.0, 1E6, true, true, "%f", 4.0));
		...
		InputParameterMap seirTransmissionMap = new InputParameterMap("SEIR", "SEIR Transmission",
				"SEIR Transmission parameters", 1.5);

		seirTransmissionMap
				.add(new InputParameterDouble("contagiousness", "contagiousness as calculated from exp-formula",
		...
		seirTransmissionMap.add(new InputParameterDouble("calculation_threshold",
				...
				"Below this contact duration, no infections will be calculated", 60, 0.0, 3600.0, true, true, "%f",
				5.0));
