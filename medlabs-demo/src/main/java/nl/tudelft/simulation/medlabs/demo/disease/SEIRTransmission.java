...
public class SEIRTransmission extends DiseaseTransmission {
	 ...
	 * p_B is the base transmission parameter without distancing, masking, or
	 * otherwise.
	 */
	 ...
	 * Beta is a correction factor for mask wearing and other personal protection;
	 * beta is in [0, 1].
	 */
	private double beta;
	 ...
	 * t_e_min is the day after exposure when a person becomes contagious for the
	 * first time.
	 */
	...
	/**
	 * Parameter to indicate when the duration is too short to make an infection
	 * calculation (converted to hours). Note that for public transport and shops
	 * the interval should not be set too short since people get in and out
	 * frequently.
	 */
	...
	/**
	 * Calculate the disease spread for all persons present in this (sub)location
	 * during the 'duration' in hours. The method could return quickly when the
	 * delta-time is very short (e.g, less than a minute but be aware that spread in
	 * public transport and shops might suffer since many people arrive and depart
	 * with short intervals).<br>
	 * <br>
	 * The formula to use to see if infectious persons j = 1..N_K infect another
	 * person i in location K is: <br>
	 * 
	 * <pre>
	 * <code>                        
	 ...
	 *                         infectious spread
	 * @param subLocationIndex int; the sublocation for which to calculate possible
	 *                         infection(s)
	 * @param duration         double; the time for which the calculation needs to
	 *                         take place, in hours
	 * @return boolean; whether a calculation took place and the last calculation
	 *         time in DiseaseTransmission can be updated
	 */
	@Override
	public boolean infectPeople(final Location location, final TIntSet personsInSublocation, final double duration) {
		...
		if (lt.isInfectInSublocation() || location.getNumberOfSubLocations() < 2) {
			...
			// NOTE: WHEN THE LOCATION HAS ONLY 1 SUBLOCATION THIS PART OF THE METHOD IS
			// USED (MUCH FASTER)
