...
public class SEIRProgression extends DiseaseProgression {
	...
	/**
	 * "S": Susceptible disease phase. This is the normal 'base' phase of every
	 * person.
	 */
	public static DiseasePhase susceptible;

	/**
	 * "E": Exposed disease phase. Note: the exposure date is stored with the
	 * Person. Exposed means the person WILL get ill.
	 */
	...
	/**
	 * I -> R period. The probability is assumed to be 1 (all asymptomatic persons
	 * recover).
	 */
	private DurationDistribution distInfectedToRecovery;

	/**
	 * Create the SEIR Progression model. A state machine is instantiated with
	 * probabilities for the state transitions and durations between states.
	 * 
	 * <pre>
	...
	public SEIRProgression(final MedlabsModelInterface model) {
		...
		this.distIncubationPeriod = new DurationDistribution(
				new DistTriangular(this.model.getRandomStream(), 24.0, 48.0, 72.0), TimeUnit.HOUR);
